...
function configurationReady() {
  const provider = settings.providers.find(p => p.id === settings.selected_provider_id);
  return Boolean(
    provider &&
    provider.api_key?.trim() &&
    provider.models.some(m => m.id === settings.selected_model_id)
  );
}
...
function renderServices() {
  ...
  for (const provider of [...settings.providers].sort((a,b)=>a.display_name.localeCompare(b.display_name))) {
    ...
    button.innerHTML = `<b>${esc(provider.display_name)}</b><small>${provider.api_key?.trim() ? '🔑' : '⊘'} ${styleName(provider.api_style)} · ${presetName(provider.optimization_preset)}</small>`;
...
function mappingRow(item, collection, container, supportsOutputMode=false) { const row=document.createElement('div'); row.className=`mapping-row${supportsOutputMode?' language-row':''}`; row.innerHTML=`<input class="display" value="${esc(item.display_name)}" placeholder="显示名称"><input class="value" value="${esc(item.prompt_value)}" placeholder="替换内容">${supportsOutputMode?`<select class="output-mode"><option value="text">普通文本</option><option value="latex">公式渲染</option></select>`:''}<button title="删除">🗑</button>`; row.querySelector('.display').oninput=e=>item.display_name=e.target.value; row.querySelector('.value').oninput=e=>item.prompt_value=e.target.value; const mode=row.querySelector('.output-mode');if(mode){mode.value=item.output_mode||'text';mode.onchange=e=>item.output_mode=e.target.value} row.querySelector('button').onclick=()=>{if(collection.length<=1)return;collection.splice(collection.indexOf(item),1);renderMappings()}; container.append(row); }
function renderMappings(){ const languages=$('#languages-list'), additions=$('#additions-list');languages.innerHTML='';additions.innerHTML='';settings.languages.forEach(item=>mappingRow(item,settings.languages,languages,true));settings.additions.forEach(item=>mappingRow(item,settings.additions,additions)); }
function hydrate(){ selectedProviderId = selectedProviderId || settings.selected_provider_id || settings.providers[0]?.id; selectedModelId = selectedModelId || settings.selected_model_id || selectedProvider()?.models[0]?.id; $('#prompt').value=settings.prompt_template; $('#launch-at-startup').checked=settings.launch_at_startup!==false; renderServices();renderMappings();validatePrompt(); }

function openProviderEditor(provider=null){ editingProviderId=provider?.id||null;$('#provider-dialog-title').textContent=provider?'编辑服务商':'添加服务商';$('#provider-name').value=provider?.display_name||'';$('#provider-url').value=provider?.base_url||'https://api.openai.com/v1';$('#provider-style').value=provider?.api_style||'responses';$('#provider-preset').value=provider?.optimization_preset||'generic';$('#provider-optimize').checked=provider?.translation_optimizations_enabled??false;$('#provider-key').value='';$('#provider-key').placeholder=provider&&provider.api_key?.trim()?'留空以保留现有密钥':'请输入 API 密钥';$('#provider-error').textContent='';$('#provider-dialog').showModal();}
...
$('#provider-dialog form').onsubmit=async event=>{event.preventDefault();const name=$('#provider-name').value.trim(),url=$('#provider-url').value.trim(),key=$('#provider-key').value;if(!name||!url){$('#provider-error').textContent='显示名称和基础网址不能为空。';return}try{let provider=settings.providers.find(p=>p.id===editingProviderId);if(!provider){provider={id:id(),display_name:name,base_url:url,api_style:'responses',optimization_preset:'generic',translation_optimizations_enabled:false,api_key:'',models:[]};settings.providers.push(provider)}provider.display_name=name;provider.base_url=url;provider.api_style=$('#provider-style').value;provider.optimization_preset=$('#provider-preset').value;provider.translation_optimizations_enabled=$('#provider-optimize').checked;if(key.trim())provider.api_key=key.trim();selectedProviderId=provider.id;selectedModelId=provider.models[0]?.id;await persistServiceChanges();renderServices();$('#provider-dialog').close()}catch(error){$('#provider-error').textContent=String(error)}};
...
$('#language-add').onclick=()=>{settings.languages.push({id:id(),display_name:'新语言或任务',prompt_value:'',output_mode:'text'});renderMappings()};$('#addition-add').onclick=()=>{settings.additions.push({id:id(),display_name:'新要求',prompt_value:''});renderMappings()};
...
$('#save-settings').onclick=async()=>{if(!validatePrompt()){document.querySelector('[data-tab="prompt"]').click();return}settings.prompt_template=$('#prompt').value;settings.selected_provider_id=selectedProviderId||null;settings.selected_model_id=selectedModelId||null;settings.launch_at_startup=$('#launch-at-startup').checked;try{await invoke('save_settings',{settings});$('#save-state').textContent='已保存';await invoke('close_settings_window')}catch(error){$('#global-error').hidden=false;$('#global-error').textContent=String(error)}};
