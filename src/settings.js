...
$('#save-settings').onclick=async()=>{if(!validatePrompt()){document.querySelector('[data-tab="prompt"]').click();return}settings.prompt_template=$('#prompt').value;settings.always_on_top=$('#always-top').checked;settings.auto_submit_enabled=$('#auto-submit-setting').checked;settings.selected_provider_id=selectedProviderId||null;settings.selected_model_id=selectedModelId||null;try{await invoke('save_settings',{settings});$('#save-state').textContent='已保存';setTimeout(()=>$('#save-state').textContent='',1800)}catch(error){$('#global-error').hidden=false;$('#global-error').textContent=String(error)}};
