...
一个使用 **Rust + Tauri 2** 编写的轻量级 Windows 全局划词翻译与数学公式渲染工具。
...
- 悬停红点自动打开翻译卡片。
- 流式显示翻译结果。
- 翻译窗口中的原文可以直接编辑，停止输入约 500 ms 后自动重新翻译。
- 支持 `Ctrl+Enter` 立即使用编辑后的原文重新翻译。
- 支持将选中的线性数学表达式转换成 LaTeX，并使用内置 KaTeX 显示排版后的公式。
- 公式模式使用独立的转录提示词，不再把 LaTeX 当作一种目标语言处理。
- 自动清理模型返回的 Markdown 代码框、`$...$`、`$$...$$` 等外层标记。
- 公式生成后可复制干净的 LaTeX 源码。
- 可在翻译卡片中选择目标语言和附加要求。
- 翻译卡片可拖动、缩放、置顶和复制译文。
- 未置顶时，点击其他程序会自动隐藏卡片。
- 系统托盘提供“设置”和“退出”入口。
...
所有设置统一从系统托盘打开。v0.7.0 会保留旧版的服务商、模型、API Key、翻译设置和开机自启动状态。
...
划词唤出翻译卡片后，顶部的原文区域是可编辑输入框。划词获得的文字仅作为初始内容，之后可以直接修改、删除或补充。

- 修改原文后，当前尚未完成的流式翻译会立即停止并从界面中清除；
- 停止输入约 500 ms 后，软件自动使用完整的新原文重新翻译；
- 连续输入期间不会每敲一个字就发送一次请求；
- 按 `Ctrl+Enter` 可以跳过等待，立即重新翻译；
- 普通 `Enter` 仍然用于插入换行；
- 中文、日文等输入法处于拼音或假名组合阶段时不会提前提交，选字完成后才开始 500 ms 计时；
- 切换目标语言、附加要求或“∑ 公式渲染”时，始终使用当前编辑框中的内容；
- 清空原文后不会发送请求，重新输入内容后恢复自动翻译；
- 原文输入框会根据实际行数完整增高，不使用内部纵向滚动条；翻译卡片随之调整，并受屏幕可用高度约束。

编辑内容只在当前翻译卡片中使用，不会覆盖其他程序中的原文字句。

## 数学公式渲染

### 直接使用

v0.6.0 的“目标语言与任务”列表默认包含：

```text
∑ 公式渲染
```

选中数学表达式并悬停红点后，在卡片顶部选择“∑ 公式渲染”。软件会：

1. 将选中文字发送给当前模型；
2. 使用独立的数学公式转录提示词生成纯 LaTeX 源码；
3. 清除模型偶尔添加的代码框和公式定界符；
4. 使用安装包内置的 KaTeX 离线渲染公式；
5. 在卡片中显示排版后的数学公式。

公式生成完成后，“复制译文”按钮会自动变成“复制 LaTeX”。点击后复制的是干净的 LaTeX 源码，便于粘贴到 LaTeX 编辑器、Markdown、Word 或 PowerPoint 的公式框中。
...
卡片中显示的则是排版后的公式，而不是上面的源码字符串。

### 自定义公式任务

打开“设置 → 变量映射”，目标项现在包含三列：
...
真正决定行为的是“输出类型”。选择“公式渲染”后，软件会忽略普通翻译提示词，自动使用内置公式转录提示词；“替换内容”仅用于辨认和兼容设置数据。

升级旧版本时，软件会自动迁移设置：如果已有名称或替换内容包含 `LaTeX`、“数学公式”或“公式渲染”的目标项，会把第一个匹配项设为公式渲染；如果没有，则自动新增“∑ 公式渲染”。软件不会擅自删除你已有的重复映射，可在设置中手动整理。

### 与 Word、PowerPoint 配合

当前“复制 LaTeX”复制的是源码，不是 Office 原生 OMML 公式对象。在 Word 或 PowerPoint 中可先按 `Alt`+`=` 建立公式框，再粘贴 LaTeX；能否直接转换由当前 Office 版本及公式输入模式决定。
...
- “目标语言”决定译文输出语言。
- 目标项的“输出类型”为“公式渲染”时，使用内置公式提示词并在结果区渲染 LaTeX；不会调用普通翻译提示词。
...
- 公式模式处理的是选区中能够读取到的纯文本。PDF 或 Office 复制时若已经丢失分数线、根号范围、矩阵布局等二维结构，模型只能根据剩余文本推断，不能保证还原正确；当前版本不包含截图公式 OCR。
- KaTeX 不支持所有 LaTeX 宏包命令。渲染失败时卡片会保留清理后的源码并显示错误，仍可复制后到完整 LaTeX 环境中检查。
...
- 原文显示区域改为可编辑输入框。
- 停止输入约 500 ms 后自动重新翻译，减少连续输入产生的重复请求。
- 支持 `Ctrl+Enter` 立即重新翻译，保留普通回车换行。
- 兼容中文、日文等输入法的组合输入过程。
- 开始编辑时立即取消旧流式响应，避免新旧译文混合并减少无效 token 消耗。
- 切换语言、附加要求或公式任务时使用编辑后的最新原文。
- 原文为空时不发送模型请求，并显示输入提示。
- 原文输入框高度和翻译卡片继续随内容自动调整。
- 修复输入框边框导致内容区少约 2 px、少量文字也可能错误显示滚动条的问题。
- 取消原文区域原有的 160 px 高度上限和内部纵向滚动条，按实际内容完整展开。
- 翻译窗口最大高度改为参考当前屏幕可用高度，避免普通长文本窗口超出桌面。
- README 与 package、Cargo、Tauri 版本统一更新为 0.7.0。

## v0.6.0 更新内容

- 新增独立的数学公式转录模式。
- 目标映射新增“普通文本/公式渲染”输出类型。
- 新安装默认提供“∑ 公式渲染”任务。
- 自动迁移 v0.5.0 及更早版本的 LaTeX/数学公式映射。
- 内置 KaTeX 及所需字体，公式预览不依赖外部网页或 CDN。
- 生成结束后自动清理代码框和公式定界符，并执行 LaTeX 语法渲染。
- 公式模式下按钮自动变为“复制 LaTeX”，复制清理后的源码。
- 渲染失败时保留源码并显示明确错误，不影响普通翻译。
- 版本号统一升级为 0.6.0。
